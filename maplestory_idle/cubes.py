...
from stage_settings import (
    ...
    get_combat_mode_from_string,
    get_fight_duration,
    get_fight_duration_from_string,
)
...
class PotentialLine:
    ...
    def get_stats(self, job_class: Optional['JobClass'] = None,
                  character_level: int = 100) -> 'StatBlock':
        """
        Convert this potential line to a StatBlock.

        Args:
            job_class: Required for MAIN_STAT_PER_LEVEL conversion
            character_level: Character level for per-level stats
        """
        from stats import StatBlock, EMPTY_STATS, create_stat_block_for_job

        if self.value == 0:
            return EMPTY_STATS

        stat_name = self.stat_type.value

        # Handle stat_per_level (needs job class)
        if self.stat_type == StatType.MAIN_STAT_PER_LEVEL:
            if job_class:
                flat_value = self.value * character_level
                return create_stat_block_for_job(job_class, main_stat_flat=flat_value)
            return EMPTY_STATS

        # Standard stat - create StatBlock with the stat name
        try:
            return StatBlock(**{stat_name: self.value})
        except TypeError:
            return EMPTY_STATS
...
class EquipmentPotential:
    """
    A set of 3 potential lines for one equipment slot (regular or bonus).
    Values are auto-calculated from tier and yellow/grey status.

    Attributes:
        slot: Equipment slot name (e.g., "hat", "gloves")
        tier: Current potential tier
        line1_stat: StatType for line 1 (always yellow)
        line2_stat: StatType for line 2
        line2_yellow: Whether line 2 is yellow (True) or grey (False)
        line3_stat: StatType for line 3
        line3_yellow: Whether line 3 is yellow (True) or grey (False)
        pity: Current pity counter for tier-up
    """
    slot: str
    tier: PotentialTier = PotentialTier.LEGENDARY
    line1_stat: Optional[StatType] = None
    line2_stat: Optional[StatType] = None
    line2_yellow: bool = True
    line3_stat: Optional[StatType] = None
    line3_yellow: bool = True
    pity: int = 0

    def get_stat_value(self, stat_type: Optional[StatType], is_yellow: bool) -> float:
        """Get auto-calculated value for a stat at current tier."""
        if stat_type is None:
            return 0.0

        effective_tier = self.tier if is_yellow else self.tier.prev_tier()
        if effective_tier is None:
            return 0.0

        # Check special potentials first
        if self.slot in SPECIAL_POTENTIALS:
            special = SPECIAL_POTENTIALS[self.slot]
            if special.stat_type == stat_type and effective_tier in special.values:
                return special.values[effective_tier]

        # Check regular potential stats
        tier_stats = POTENTIAL_STATS.get(effective_tier, [])
    ...
    @property
    def line1_value(self) -> float:
        """Auto-calculated value for line 1 (always yellow)."""
        return self.get_stat_value(self.line1_stat, is_yellow=True)

    @property
    def line2_value(self) -> float:
        """Auto-calculated value for line 2."""
        return self.get_stat_value(self.line2_stat, self.line2_yellow)

    @property
    def line3_value(self) -> float:
        """Auto-calculated value for line 3."""
        return self.get_stat_value(self.line3_stat, self.line3_yellow)

    def get_lines(self) -> List[PotentialLine]:
        """Get all 3 lines as PotentialLine objects."""
        return [
            PotentialLine(slot=1, stat_type=self.line1_stat or StatType.DEX_PCT,
                         value=self.line1_value, is_yellow=True),
            PotentialLine(slot=2, stat_type=self.line2_stat or StatType.DEX_PCT,
                         value=self.line2_value, is_yellow=self.line2_yellow),
            PotentialLine(slot=3, stat_type=self.line3_stat or StatType.DEX_PCT,
                         value=self.line3_value, is_yellow=self.line3_yellow),
        ]

    def get_stats(self, job_class: Optional['JobClass'] = None,
                  character_level: int = 100) -> 'StatBlock':
        """Get combined stats from all lines as a StatBlock."""
        from stats import EMPTY_STATS
        total = EMPTY_STATS
        for line in self.get_lines():
            if line.stat_type and line.value > 0:
                total = total + line.get_stats(job_class, character_level)
        return total

    def to_dict(self) -> Dict:
        """Serialize to dict for storage."""
        return {
            "tier": self.tier.value if isinstance(self.tier, PotentialTier) else self.tier,
            "pity": self.pity,
            "line1_stat": self.line1_stat.value if self.line1_stat else "",
            "line2_stat": self.line2_stat.value if self.line2_stat else "",
            "line2_yellow": self.line2_yellow,
            "line3_stat": self.line3_stat.value if self.line3_stat else "",
            "line3_yellow": self.line3_yellow,
        }

    @classmethod
    def from_dict(cls, slot: str, data: Dict) -> 'EquipmentPotential':
        """Create from dict."""
        # Parse tier
        tier_val = data.get("tier", "legendary")
        if isinstance(tier_val, str):
            try:
                tier = PotentialTier(tier_val.lower())
            except ValueError:
        ...
        # Parse stat types
        def parse_stat(val) -> Optional[StatType]:
            if not val:
                return None
            try:
                return StatType(val)
        ...
        return cls(
            slot=slot,
            tier=tier,
            line1_stat=parse_stat(data.get("line1_stat")),
            line2_stat=parse_stat(data.get("line2_stat")),
            line2_yellow=data.get("line2_yellow", True),
            line3_stat=parse_stat(data.get("line3_stat")),
            line3_yellow=data.get("line3_yellow", True),
            pity=int(data.get("pity", 0)),
        )


@dataclass
class SlotPotentials:
    """Both regular and bonus potentials for one equipment slot."""
    slot: str
    regular: EquipmentPotential = None
    bonus: EquipmentPotential = None

    def __post_init__(self):
        if self.regular is None:
            self.regular = EquipmentPotential(slot=self.slot)
        if self.bonus is None:
            self.bonus = EquipmentPotential(slot=self.slot)

    def get_stats(self, job_class: Optional['JobClass'] = None,
                  character_level: int = 100) -> 'StatBlock':
        """Get combined stats from both regular and bonus potentials."""
        return (
            self.regular.get_stats(job_class, character_level) +
            self.bonus.get_stats(job_class, character_level)
        )

    def to_dict(self) -> Dict:
        """Serialize to dict."""
        result = self.regular.to_dict()
        result["regular_pity"] = result.pop("pity")

        bonus = self.bonus.to_dict()
        result["bonus_tier"] = bonus["tier"]
        result["bonus_pity"] = bonus["pity"]
        for key in ["line1_stat", "line2_stat", "line2_yellow",
                    "line3_stat", "line3_yellow"]:
            result[f"bonus_{key}"] = bonus[key]

        return result

    @classmethod
    def from_dict(cls, slot: str, data: Dict) -> 'SlotPotentials':
        """Create from dict."""
        # Regular potential
        regular_data = {
            "tier": data.get("tier", "legendary"),
            "pity": data.get("regular_pity", 0),
            "line1_stat": data.get("line1_stat", ""),
            "line2_stat": data.get("line2_stat", ""),
            "line2_yellow": data.get("line2_yellow", True),
            "line3_stat": data.get("line3_stat", ""),
            "line3_yellow": data.get("line3_yellow", True),
        }

        # Bonus potential
        bonus_data = {
            "tier": data.get("bonus_tier", "legendary"),
            "pity": data.get("bonus_pity", 0),
            "line1_stat": data.get("bonus_line1_stat", ""),
            "line2_stat": data.get("bonus_line2_stat", ""),
            "line2_yellow": data.get("bonus_line2_yellow", True),
            "line3_stat": data.get("bonus_line3_stat", ""),
            "line3_yellow": data.get("bonus_line3_yellow", True),
        }

        return cls(
            slot=slot,
            regular=EquipmentPotential.from_dict(slot, regular_data),
            bonus=EquipmentPotential.from_dict(slot, bonus_data),
        )
...
# Import shared display functions from constants
from constants import (
    get_stat_display_name as get_stat_display_name_str,
    get_stat_short_name as get_stat_short_name_str,
    format_stat_value as format_stat_value_str,
    Rarity,
    RARITY_COLORS,
...
# Potential tier colors (same as rarity but using PotentialTier enum)
TIER_COLORS: Dict[PotentialTier, str] = {
    PotentialTier.NORMAL: "#888888",
    PotentialTier.RARE: "#5599ff",
    PotentialTier.EPIC: "#cc77ff",
    PotentialTier.UNIQUE: "#ffcc00",
    PotentialTier.LEGENDARY: "#66ff66",
    PotentialTier.MYSTIC: "#ff6666",
}

TIER_ABBREVIATIONS: Dict[PotentialTier, str] = {
    PotentialTier.NORMAL: "NOR",
    PotentialTier.RARE: "RAR",
    PotentialTier.EPIC: "EPC",
    PotentialTier.UNIQUE: "UNI",
    PotentialTier.LEGENDARY: "LEG",
    PotentialTier.MYSTIC: "MYS",
}
...
def get_tier_abbreviation(tier: PotentialTier) -> str:
    """Get 3-letter abbreviation for tier."""
    return TIER_ABBREVIATIONS.get(tier, "???")


def get_stat_display_name(stat_type: StatType) -> str:
    """Get full human-readable stat name from StatType enum."""
    return get_stat_display_name_str(stat_type.value)


def get_stat_short_name(stat_type: StatType) -> str:
    """Get short display name for stat (for compact UI)."""
    return get_stat_short_name_str(stat_type.value)


def format_stat_value(stat_type: StatType, value: float) -> str:
    """Format a stat value with appropriate suffix."""
    return format_stat_value_str(stat_type.value, value)
...
def format_line(line: PotentialLine) -> str:
    ...
    val_str = format_stat_value(line.stat_type, line.value)
    return f"[{color}] {name}: {val_str}{special}"


def get_tier_from_string(tier_str: str) -> PotentialTier:
    """Convert tier string to PotentialTier enum."""
    try:
        return PotentialTier(tier_str.lower())
    except ValueError:
        return PotentialTier.LEGENDARY


def get_available_stats_for_slot(slot: str) -> List[StatType]:
    """
    Get list of available stat types for a slot.
    Special potential stat is first if available for this slot.
    """
    # Base stats available on all slots
    base_stats = [
        StatType.DEX_PCT, StatType.STR_PCT, StatType.INT_PCT, StatType.LUK_PCT,
        StatType.DEX_FLAT, StatType.STR_FLAT, StatType.INT_FLAT, StatType.LUK_FLAT,
        StatType.DAMAGE_PCT, StatType.CRIT_RATE, StatType.MIN_DMG_MULT, StatType.MAX_DMG_MULT,
        StatType.ATTACK_SPEED, StatType.DEFENSE, StatType.MAX_HP, StatType.MAX_MP,
    ]

    # Add special stat for this slot at the beginning
    if slot in SPECIAL_POTENTIALS:
        special_stat = SPECIAL_POTENTIALS[slot].stat_type
        return [special_stat] + base_stats

    return base_stats


def get_stat_value_at_tier(
    stat_type: StatType,
    tier: PotentialTier,
    slot: str,
    is_yellow: bool = True
    ...
    """
    Get the value of a stat at a given tier.
    Handles both regular and special potentials.
    Grey lines use the previous tier's value.
    """
    effective_tier = tier if is_yellow else tier.prev_tier()
    if effective_tier is None:
        return 0.0

    # Check special potential first
    if slot in SPECIAL_POTENTIALS:
...
def get_special_stat_for_slot(slot: str) -> Optional[StatType]:
    """Get the special stat type for a slot, or None if no special."""
    if slot in SPECIAL_POTENTIALS:
        return SPECIAL_POTENTIALS[slot].stat_type
    return None
